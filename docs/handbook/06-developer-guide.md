...
metaphor migration generate Invoice __MODULE__
metaphor migration run
...
(`__MODULE__` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed __MODULE__          # run Rust seeders
metaphor migration generate-seeds __MODULE__  # emit SQL seed files
```
