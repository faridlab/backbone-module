...
metaphor make entity Vendor --module __MODULE__
...
metaphor schema schema validate __MODULE__

# 3. Generate all artifacts (entity, DTOs, repo, service, handler, routes).
metaphor schema schema generate __MODULE__ --target all --force

# 4. Generate the migration for the new entity.
metaphor migration generate Vendor __MODULE__
...
> `__MODULE__` is your module name (auto-detected from the current directory when omitted).
...
2. `metaphor schema schema validate __MODULE__`.
3. Generate a migration for the change:
   `metaphor migration generate <Entity> __MODULE__` (or, for a schema-diff-driven migration against
   a live DB, `metaphor schema schema migration __MODULE__ --database-url …`).
4. Regenerate code: `metaphor schema schema generate __MODULE__ --target all --force`.
